...
> {
  ...
  constructor(props: any) {
    ...
    const session = props.fancies
    const updateFancy: Record<string, IFancy> = {}
    const typesFancy: any = this.groupFanciesByGtype(session)
    console.log(session,"session from the frrontend  Hello Worrld ")
    console.log(typesFancy, "Grouped fancies by gtype")
    
    session.forEach((fancy: LFancy) => {
      updateFancy[fancy.marketId] = {
        gtype: fancy.gtype as FancyType,
        SelectionId: fancy.marketId,
        RunnerName: fancy.fancyName,
  ...
  componentDidUpdate(prevProps: Readonly<{ fancies: LFancy[]; matchId: string }>): void {
    ...
    if (prevProps.fancies !== this.props.fancies) {
      const typesFancy: any = this.groupFanciesByGtype(this.props.fancies)
      this.setState({ fancies: typesFancy })
    }
  }
  ...
  addNewFancy = () => {
    if (this.props.socketUser) {
      this.props.socketUser.on('addNewFancy', ({ newFancy, fancy }: any) => {
        const gtype = newFancy.gtype || 'session'
        const ballType = this.FancyBallTypes(fancy.RunnerName)
        
        const currentGtypeFancies = this.state.fancies[gtype] || {
          blank: [],
          ballRun: [],
          overBallNo: []
        }
        const allFacies = [...currentGtypeFancies[ballType]]

        if (this.checkFancyKey(allFacies, fancy) === -1) {
          const items = { ...this.state.fancyUpdate }
          allFacies.push(newFancy)
          this.setState({
            fancies: {
              ...
              [gtype]: {
                ...currentGtypeFancies,
                [ballType]: allFacies.sort((a: any, b: any) => a.sr_no - b.sr_no),
              }
            },
            fancyUpdate: { ...items, [fancy.SelectionId]: fancy },
          })
  ...
  removeFancy = () => {
    if (this.props.socketUser)
      this.props.socketUser.on('removeFancy', (fancy: any) => {
        const gtype = fancy.gtype || 'session'
        const ballType = this.FancyBallTypes(fancy.fancyName || fancy.RunnerName)
        
        const currentGtypeFancies = this.state.fancies[gtype]
        if (!currentGtypeFancies) return
        
        const allFacies = [...currentGtypeFancies[ballType]]
        const filteredFancies = allFacies.filter((f) => f.marketId !== fancy.marketId)
  ...
  suspendedFancy = () => {
    this.context.socket.on('suspendedFancy', ({ fancy, type }) => {
      const gtype = fancy.gtype || 'session'
      const ballType = this.FancyBallTypes(fancy.fancyName)
      
      const currentGtypeFancies = this.state.fancies[gtype]
      if (!currentGtypeFancies) return
      
      const fancies = [...currentGtypeFancies[ballType]]
      const fanciesIndex = fancies.findIndex(
        (f) => f.marketId === fancy.marketId && f.matchId === fancy.matchId,
      )
  ...
  groupFanciesByGtype(fancies: LFancy[]) {
    const groupedFancies: any = {}
    ...
    fancies
      ...
      .forEach((fancy: LFancy) => {
        ...
        if (!groupedFancies[gtype]) {
          groupedFancies[gtype] = {
            blank: [],
            ballRun: [],
            overBallNo: []
          }
        ...
        groupedFancies[gtype][ballType].push(fancy)
      })
    
    return groupedFancies
  }
  ...
  getGtypeLabel(gtype: string): string {
    const gtypeLabels: Record<string, string> = {
      'session': 'Session Market',
      'fancy1': 'Fancy1 Market',
      'oddeven': 'Odd/Even Market',
    ...
    return gtypeLabels[gtype] || `${gtype} Market`
  }

  onFancyType = (type: string) => {
    store.dispatch(setFancyType(type))
    this.setState({ fancies: {} as FancyByGtype })
  }
    ...
    return (
      <>
        ...
        <div
          ...
          style={{ marginLeft: isMobile ? '0px' : '', marginRight: isMobile ? '0px' : '' }}
        >
          {Object.keys(fancies).map((gtype) => {
            const gtypeFancies = fancies[gtype]
            const hasBlank = gtypeFancies?.blank && gtypeFancies.blank.length > 0
            const hasOverBallNo = gtypeFancies?.overBallNo && gtypeFancies.overBallNo.length > 0
            const hasBallRun = gtypeFancies?.ballRun && gtypeFancies.ballRun.length > 0
                ...
                {hasBlank && (
                  <div className={`${clsgrid}`} style={{ padding: isMobile ? '0px' : '' }}>
                    <div>
                      ...
                      {this.fancyheader(this.getGtypeLabel(gtype))}
                      <FancyList fancies={gtypeFancies.blank} fancyUpdate={fancyUpdate} />
                    </div>
                  </div>
                )}
                ...
                {hasOverBallNo && (
                  <div className={`${clsgrid}`} style={{ padding: isMobile ? '0px' : '' }}>
                    <div>
                      {!isMobile && (
                        <div className='market-title mt-1'>
                          {this.getGtypeLabel(gtype)} - Over by Over
                        </div>
                      )}
                      {this.fancyheader(`${this.getGtypeLabel(gtype)} - Over by Over`)}
                      <FancyList fancies={gtypeFancies.overBallNo} fancyUpdate={fancyUpdate} />
                    </div>
                  </div>
                )}
                ...
                {hasBallRun && (
                  <div className={`${clsgrid}`} style={{ padding: isMobile ? '0px' : '' }}>
                    <div>
                      {!isMobile && (
                        <div className='market-title mt-1'>
                          {this.getGtypeLabel(gtype)} - Ball by Ball
                        </div>
                      )}
                      {this.fancyheader(`${this.getGtypeLabel(gtype)} - Ball by Ball`)}
                      <FancyList fancies={gtypeFancies.ballRun} fancyUpdate={fancyUpdate} />
                    </div>
                  </div>
                )}
              </React.Fragment>
